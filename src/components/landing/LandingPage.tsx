        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col gap-4 text-center">
              ...
              <button onClick={() => { onOpenAuth(); setIsOpen(false); }}
          ...
          <div className="grid lg:grid-cols-2 gap-16 md:gap-24 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
              ...
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold tracking-tight mb-8 md:mb-10 leading-[1.05] dark:text-white">
                Your Exam <br/><span className="text-brand">On Autopilot.</span>
              </h1>
              ...
              <div className="mt-12 md:mt-20 glass p-6 md:p-8 rounded-3xl shadow-sm flex items-center gap-4 md:gap-6 max-w-md">
                ...
                <div>
                  <p className="text-sm md:text-base italic text-slate-500 dark:text-slate-400 leading-tight">&ldquo;Kept me organized and productive through the most intense semester of my life.&rdquo;</p>
                  <p className="text-[10px] font-semibold tracking-wide text-brand mt-2">Arjun S., University Junior</p>
                </div>
              </div>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 50, rotate: 2 }} animate={{ opacity: 1, y: 0, rotate: 0 }} transition={{ duration: 1, delay: 0.2 }} className="relative hidden lg:block">
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-xl shadow-brand/5 border border-slate-100 dark:border-slate-800 flex flex-col">
                  <div className="w-12 h-12 bg-brand/10 rounded-2xl flex items-center justify-center mb-5"><Zap className="w-6 h-6 text-brand" /></div>
                ...
                <div className="bg-brand rounded-3xl p-8 shadow-xl shadow-brand/20 flex flex-col text-white transform translate-y-12">
                  <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mb-5"><BookOpen className="w-6 h-6" /></div>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 shadow-xl shadow-brand/5 border border-slate-100 dark:border-slate-800 flex flex-col translate-x-4">
                  <div className="w-full h-24 bg-slate-50 dark:bg-slate-800 rounded-2xl mb-5 flex items-end gap-2 p-4">
                    <div className="flex-1 bg-brand/20 dark:bg-brand/30 rounded-t-xl" style={{ height: '40%' }} />
                    <div className="flex-1 bg-brand/40 dark:bg-brand/50 rounded-t-xl" style={{ height: '70%' }} />
                    <div className="flex-1 bg-brand/30 dark:bg-brand/40 rounded-t-xl" style={{ height: '55%' }} />
                    <div className="flex-1 bg-brand/60 dark:bg-brand/70 rounded-t-xl" style={{ height: '90%' }} />
                  </div>
      ...
      <section id="features" className="py-20 md:py-28">
        <div className="container mx-auto px-6 md:px-12">
          <div className="max-w-4xl mb-16 md:mb-20">
            ...
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-display font-bold mb-4 md:mb-6 dark:text-white tracking-tight">Everything you need <br/><span className="text-brand">to succeed.</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            <FeatureCard icon={Zap} title="Dynamic Flows" description="Get a personalized schedule that adapts to your speed and changing deadlines automatically." delay={0.1} />
            <FeatureCard icon={BookOpen} title="Syllabus Maps" description="Break down complex syllabi into manageable daily study goals based on high-yield topics." delay={0.2} />
            <FeatureCard icon={Calendar} title="Workspace Sync" description="Seamlessly sync your study sessions with Google Calendar and Tasks to stay organized across devices." delay={0.3} />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid lg:grid-cols-3 gap-10 items-end">
            <div className="lg:col-span-1 mb-10 lg:mb-0">
              ...
              <h2 className="text-3xl md:text-5xl font-display font-bold mb-4 md:mb-6 dark:text-white tracking-tight">Focus on <br/><span className="text-brand">The Grade,</span> <br/>Not the cost.</h2>
              ...
              <div className="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center flex-shrink-0"><CheckCircle className="w-5 h-5" /></div>
            ...
            <div className="lg:col-span-2 grid md:grid-cols-2 gap-5 md:gap-6">
              <div className="p-8 md:p-10 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all">
                ...
                <div className="flex items-end gap-1 mb-6 md:mb-8"><div className="text-5xl md:text-7xl font-display font-bold leading-[0.8] dark:text-white tracking-tight">Free</div></div>
                <ul className="space-y-3 md:space-y-4 mb-8 md:mb-10">
                  {['Single Study Plan', 'Web Dashboard', 'Sync to Tasks', 'Standard Templates'].map(item => (
                    <li key={item} className="flex items-center gap-3 text-sm md:text-base font-medium text-slate-500 dark:text-slate-400"><div className="w-1.5 h-1.5 rounded-full bg-brand shrink-0" /> {item}</li>
                  ))}
                </ul>
                <button onClick={onOpenAuth} className="w-full py-4 rounded-2xl border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-white font-semibold text-sm hover:bg-slate-900 hover:text-white hover:border-slate-900 transition-all">Sign Up</button>
              </div>
              <div className="p-8 md:p-10 rounded-3xl bg-slate-900 dark:bg-slate-800 text-white shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-6"><Zap className="w-6 h-6 md:w-7 md:h-7 text-brand-light" /></div>
                <h3 className="text-[9px] font-semibold tracking-wide text-brand-light mb-6 md:mb-8">Pro Plan</h3>
                <div className="flex items-end gap-1 mb-6 md:mb-8"><div className="text-5xl md:text-7xl font-display font-bold leading-[0.8] tracking-tight">$9</div><div className="text-sm md:text-base font-medium text-slate-500 mb-1">/mo</div></div>
                <ul className="space-y-3 md:space-y-4 mb-8 md:mb-10">
                  {['AI Adaptive Scheduling', 'Google Calendar Sync', 'Priority Support', 'Full Analytics Panel', 'Progress Export'].map(item => (
                    <li key={item} className="flex items-center gap-3 text-sm md:text-base font-medium"><CheckCircle className="w-4 h-4 text-brand-light shrink-0" /> {item}</li>
                  ))}
                </ul>
                <button onClick={onOpenAuth} className="w-full py-4 rounded-2xl bg-brand text-white font-semibold text-sm shadow-xl shadow-brand/30 hover:scale-[1.02] transition-transform">Go Pro Now</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="faq" className="py-20 md:py-28 bg-white dark:bg-slate-900/30">
        <div className="container mx-auto px-6 md:px-12">
          <div className="grid lg:grid-cols-2 gap-10 md:gap-16">
            <div>
              <h2 className="text-3xl md:text-5xl lg:text-6xl font-display font-bold mb-6 md:mb-8 dark:text-white tracking-tight">Frequently Asked <br/><span className="text-brand">Questions.</span></h2>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-12 shadow-sm border border-slate-100 dark:border-slate-800">
              <FAQItem question="How does Google Calendar sync work?" answer="Once you generate a plan in your dashboard, you can click 'Sync to Google Apps'. StudyFlow will create a dedicated Task List and add key study milestones to your Primary Calendar automatically." />
              <FAQItem question="Can I customize subjects for specific exams?" answer="Yes, when creating a new plan you can list any subjects. Our algorithm will distribute study time based on the number of subjects and your specific exam date (e.g. Finals or Certifications)." />
              <FAQItem question="What happens if I miss a study day?" answer="The AI scheduler is dynamic. It will automatically re-allocate your missed sessions into your remaining available slots while maintaining optimal break ratios." />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 md:px-12">
          <div className="bg-slate-900 dark:bg-brand/10 rounded-3xl md:rounded-4xl p-12 md:p-24 text-center text-white relative overflow-hidden group">
            <div className="relative z-10">
              <h2 className="text-3xl md:text-6xl lg:text-7xl font-display font-bold mb-8 md:mb-10 tracking-tight leading-[1.05]">Ace Your Exams. <br/><span className="text-brand-light italic">No Excuses.</span></h2>
              <button onClick={onOpenAuth} className="bg-white text-slate-900 px-10 py-5 rounded-2xl font-semibold text-lg md:text-xl hover:scale-105 transition-all shadow-2xl">Join StudyFlow Today</button>
            </div>
